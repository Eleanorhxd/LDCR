...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.denoise_fn = denoise_fn# 没有伪标签
        # self.denoise_fn_model = denoise_fn  # rename original unet # 伪标签使用
        ...
        if schedule_opt is not None:
            pass
        # xinjia
        # self.cond_proj = nn.Linear(1024, 3)
        
        
         # xinjia
#     def denoise_fn(self, x, t, cond_emb=None):
#         """
#         调用底层 unet 去噪，并在有伪标签引导时注入额外条件。
#         cond_emb shape: [B, C] or [B, C, 1, 1]
#         """
#         latent = self.denoise_fn_model(x, t)  # 原始 U-Net 逻辑
#         # print('latent.shape:', latent.shape)([16, 1024, 1, 1])
#         # if cond_emb is not None:
#         #     if cond_emb.dim() == 2:
#         #         cond_emb = cond_emb.unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
#         #         print('cond_emb.shape:', cond_emb.shape)
#         #     latent = latent + cond_emb  # 引导加权
        
#         if cond_emb is not None:
#             # print("cond_emb shape before projection:", cond_emb.shape)

#         # cond_emb: [B, 1024, 1, 1] → [B, 1024]
#             cond_emb = cond_emb.squeeze(-1).squeeze(-1)  # 去掉最后两个维度
#             cond_emb = self.cond_proj(cond_emb)  # 线性映射到 C=3
#             cond_emb = cond_emb[:, :, None, None] 
#             latent = latent + cond_emb  # broadcast 加法
#         return latent
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):# 不使用伪标签引导
        # x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, t))
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, t))
        # 使用伪标签引导
    # def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None, cond_emb=None):
    #     # x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, t))
    #     x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, t, cond_emb))
        if clip_denoised:
            x_recon.clamp_(-1., 1.)
    ...
    # @torch.no_grad()
    # def p_sample(self, x, t, clip_denoised=True, repeat_noise=False, condition_x=None, cond_emb=None):
    def p_sample(self, x, t, clip_denoised=True, repeat_noise=False, condition_x=None):# 不使用伪标签引导
        b, *_, device = *x.shape, x.device
        # model_mean, _, model_log_variance = self.p_mean_variance(
        #     x=x, t=t, clip_denoised=clip_denoised, condition_x=condition_x)
        #new
        model_mean, _, model_log_variance = self.p_mean_variance(
            # x=x, t=t, clip_denoised=clip_denoised, cond_emb=cond_emb) # 使用伪标签引导
    ...
    # def p_sample_segment(self, x_in, opt, cond_emb=None):
            ...
            if 'STARE' in opt['datasets']['test']['dataroot'] or 'DRIVE' in opt['datasets']['test']['dataroot']:
                for opt1 in range(0,dsize,256):
                    for opt2 in range(0,dsize,256):
                        ...
                        # x_latent = self.denoise_fn(x_start, t, cond_emb)
    ...
    # def p_losses(self, x_in, noise=None, cond_emb=None):
        a_start = x_in['A']
        ...
        # A_latent = self.denoise_fn(A_noisy, t_a, cond_emb)
        ...
        # B_latent = self.denoise_fn(B_noisy, t_b, cond_emb)
        ...
        # f_recon = self.denoise_fn(f_noisy, t_a, cond_emb)
