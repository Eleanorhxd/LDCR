...
class UNet(nn.Module):
    ...
    # def forward(self, x, time, **kwargs): # 加入伪标签引导的时候使用
    ...
    # # 如果提供了伪标签嵌入，就加到最终输出上（或融合到中间）
    #     if 'cond_emb' in kwargs and kwargs['cond_emb'] is not None:
    #         cond_emb = kwargs['cond_emb']  # shape: (B, C)
    #         cond_emb = cond_emb.unsqueeze(-1).unsqueeze(-1)  # -> (B, C, 1, 1)
    #         recon = recon + cond_emb  # broadcast to (B, C, H, W)
